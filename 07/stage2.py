...
CARD_RANKS = {
  ...
  '9': 9,
  'T': 10,
  'J': 1, # J is now worth 1 for comparison purposes
  'Q': 12,
  'K': 13,
  'A': 14
}
...
def upgrade_cards(cards):
  """
  replace the jokers with whatever is already the most common card
  """
  c = Counter(cards)
  most_common_card = c.most_common(1)[0][0]
  # take the next most common if J itself is the most common
  if most_common_card == 'J':
    try:
      most_common_card = c.most_common(2)[1][0]
    # if it's all J then make them all aces
    except IndexError:
      return 'AAAAA'
